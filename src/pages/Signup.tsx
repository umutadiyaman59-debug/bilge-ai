...
const Signup: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setError('Google ile kayit yapilamadi. Lutfen tekrar deneyin.');
    } finally {
  ...
  if (success) {
    return (
      <AuthLayout
        ...
        subtitle="Email adresinize dogrulama linki gonderdik"
      >
        <div className="text-center space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Lutfen <strong>{email}</strong> adresine gonderilen linke tiklayarak hesabinizi dogrulayin.
          </p>
          <Button
            variant="outline"
            className="w-full"
  ...
  return (
    <AuthLayout
      title="Hesap Olustur"
      subtitle="Bilge'yi kullanmaya baslayin"
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {isGoogleLoading ? (
            <>
              <Loader2 className="h-4 w-4 mr-2 animate-spin" />
              Google ile Kayit Yapiliyor...
            </>
          ) : (
            <>
              <GoogleIcon />
              <span className="ml-2">Google ile Kayit Ol</span>
            </>
          )}
        </Button>
